...
function Invoke-SelfCheckMode {
    ...
    try {
        ...
        if ([System.IO.File]::ReadAllText($commentedSource) -notmatch '@GameTest' -or $testReceipts.UnitCount -ne 88 -or $testReceipts.GameTestCount -ne 55) {
            throw 'Comment-only source affected receipt-derived execution counts or the reviewed 88/55 manifest drifted.'
        }
